...
export default function DetailAdmin({ sid, user, status }: { sid: string; user: iAdminSession | null, status: "authenticated" | "loading" | "unauthenticated"; }) {
  ...
  const [GetIsRedirect, SetIsRedirect] = useState(true);
  ...
  const { mutate, isError, isSuccess, isPending, error, reset } = useMutation({
    mutationFn: async (body: any) => {
      const respomse = await axiosInstance.put(`/admin/${sid}`, body);
      return respomse;
    },
  ...
  return (
    <div>
      ...
      <ButtonBack type="button" className="mt-3 py-2 px-3 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
        KEMBALI
      </ButtonBack>
    </div>
  );
